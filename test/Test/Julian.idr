...
julianRoundTrips current final =
  if current > final
    then True
    else case refineJulianDays current of
      Left _ => False
      Right date =>
        let (valueYear, valueMonth, valueDay) = jymd date
         in case refineJulianDate valueDay valueMonth valueYear of
              Left _ => False
              Right rebuilt =>
...
mixedJulianResult = applyPeriod (months 1 <+> hours 2)
  (on (localTime 23 30 0 0) (julianDate 31 JulianMonths.January 1900))
